...
export const beneficiaryMachine = createMachine({
  ...
  actions: {
    assignReasonToContext: assign((ev, ctx) => { return { reason: ev.reason } }),
    assignAmountToContext: assign((ev, ctx) => { return { amount: ev.amount } }),
    assignEnabledToContext: assign((ev, ctx) => {
      console.log(`assignEnabledToContext EVENT -> `, ev)
      return { enabled: true }
    }),
    assignDisabledToContext: assign((ev, ctx) => {
